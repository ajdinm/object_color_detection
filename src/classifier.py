...
def read_data():
    # Reads .csv data for RGB color dataset
    ...
    for f in files:
        with open(data_folder + f) as csvfile:
            ...
            for row in reader:
                try:
                    data.append(
                        [name, map(lambda x: int(x), row[1].split(';'))])
                except:
                    continue
    return data
...
def reverse_lookup(predicted):
    lookup = [['red', [1, 0, 0, 0]], ['green', [0, 1, 0, 0]],
              ['yellow', [0, 0, 1, 0]], ['blue', [0, 0, 0, 1]]]
...
def get_train_validation_data():
    ...
    colors = map(lambda color: filter(
        lambda x: x[0] == color, data), color_names)
    ...
    x = map(lambda xi: normalize_rgb(
        xi[1]), x)  # put everything in [0, 1] range
    ...
    for i in range(len(color_names)):
        ...
        validation_y = validation_y + \
            [color_names[i]] * (len(colors[i]) - min_len)
...
def train_classifier():
    ...
    clf = MLPClassifier(solver='lbfgs', alpha=1e-5,
                        hidden_layer_sizes=(5, 3), random_state=1, activation='relu')
...
def save_classifier(clf, filename='classifier.pkl'):
